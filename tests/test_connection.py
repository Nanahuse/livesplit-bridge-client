...
from livesplit_bridge import connection as connection_module
from livesplit_bridge import events as events_module

from .test_client import FakeContext, FakeSocket, encoded_response
...
def test_single_shared_context_creates_subscriber_before_client() -> None:
    ...
    connection = BridgeConnection(context=context)

    assert context.socket_types == [zmq.SUB, zmq.REQ]
    assert (zmq.SUBSCRIBE, b"") in sub_socket.options
    assert req_socket.endpoint == connection.client.rpc_endpoint
    connection.close()
...
def test_rpc_operations_delegate_to_client() -> None:
    ...
    connection = BridgeConnection(context=FakeContext(sub_socket, req_socket))

    assert connection.snapshot() == expected
    assert connection.start().success
...
def test_receive_and_iteration_use_current_subscriber() -> None:
    ...
    connection = BridgeConnection(context=FakeContext(sub_socket, FakeSocket()))

    assert connection.receive() == first
    assert iter(connection) is connection
    assert next(connection) == second
    connection.close()
...
def test_receive_timeout_is_forwarded() -> None:
    ...
    connection = BridgeConnection(
        context=FakeContext(sub_socket, FakeSocket()), receive_timeout_ms=7
    )
...
def test_resynchronize_keeps_new_subscriber_when_snapshot_fails() -> None:
    ...
    assert old_sub.closed
    assert not new_sub.closed
    assert connection.subscriber is not None
...
def test_resynchronize_recovers_from_event_stream_loss(
    ...
    clock.now = 0.18
    assert connection.resynchronize() == snapshot
    assert connection.receive() == heartbeat
    connection.close()
...
def test_owned_context_is_terminated_on_close(monkeypatch: pytest.MonkeyPatch) -> None:
    context = FakeContext(FakeSocket(), FakeSocket())
    monkeypatch.setattr(connection_module.zmq, "Context", lambda: context)
...
def test_resynchronize_keeps_new_subscriber_when_old_close_fails() -> None:
    ...
    req_socket = FakeSocket()
    new_sub = FakeSocket()
    connection = BridgeConnection(context=FakeContext(old_sub, req_socket, new_sub))
    old_subscriber = connection.subscriber

    with pytest.raises(RuntimeError, match="close boom"):
        connection.resynchronize()

    assert connection.subscriber is not old_subscriber
    assert not new_sub.closed
    connection.close()
...
def test_partial_initialization_failure_closes_created_resources() -> None:
    ...
    with pytest.raises(IndexError):
        BridgeConnection(context=context)
